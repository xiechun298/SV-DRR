...
from dataset import LidcidriHardFBData
...
def log_validation(
    ...
    for valid_step, batch in tqdm(enumerate(validation_dataloader)):
        if (
            not validate_all_data
            and args.num_validation_batches is not None
            and valid_step >= args.num_validation_batches
        ):
            break
        gt_image = batch["image_target"].to(
            device=accelerator.device, dtype=weight_dtype
        )
        input_image = batch["image_cond"].to(
            device=accelerator.device, dtype=weight_dtype
        )
        pose = batch["T"].to(device=accelerator.device, dtype=weight_dtype)
        # images = []
        h, w = input_image.shape[2:]
        input_image = input_image.repeat(args.num_validation_images, 1, 1, 1)
        pose = pose.repeat(args.num_validation_images, 1)
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            ...
            for log_id, log in enumerate(image_logs):
                ...
                pid = log["pid"][0]  # pid
                target_view = log["target_view"][0]  # target_view
                pose = log["pose"]  # pose
                theta = math.degrees(pose[0].item())
                phi = math.degrees(math.atan2(pose[1].item(), pose[2].item()))

                formatted_images.append(
                    wandb.Image(input_image, caption="{}_input {}".format(log_id, pid))
                )
                formatted_images.append(
                    wandb.Image(
                        gt_image,
                        caption="{}_gt ({}:{:.2f},{:.2f})".format(
                            log_id, target_view, theta, phi
                        ),
                    )
                )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to differ the seed on each device slightly with `self.process_index`.",
    )
    parser.add_argument(
        "--use_seedable_sampler",
        action="store_true",
        help="Whether to use a seedable sampler for the dataloader.",
    )
    parser.add_argument(
        "--resolution",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="thin",
        help=("The thickness of the CT scans. Choose between `thin`, `thick`, `all`."),
    )
    parser.add_argument(
        "--xray_orientation",
        type=str,
        default="PA",
        help=(
            "The orientation of the camera views for training. Choose between `PA`, `AP`, `all`."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of images to be generated for each image-pose pair during validation.",
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help=("Number of batches to use for validation. If `None`, use all batches."),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("The `run_id` of the wandb run log to resume."),
    )
    parser.add_argument(
        "--run_compile",
        action="store_true",
        help=(
            "Whether to run the `torch.compile` function to optimize the model for faster inference."
        ),
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Whether to run in debug mode (more verbose logging).",
    )
    parser.add_argument(
        "--use_additional_conditions",
        type=bool,
        default=False,
        help="Whether to use additional conditions. If not set, true for 1024 input size and false otherwise.",
    )
...
def _encode_image(
    ...
    encoder_output = image_encoder(image, output_hidden_states=True)
    # last_hidden_state = encoder_output.last_hidden_state [B,257,1024]
    image_embeddings = encoder_output.image_embeds.to(dtype=dtype)
...
def main(args):
    ...
    if args.seed is not None:
        set_seed(args.seed, args.device_specific_seed)
    ...
    transformer = SvdrrTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path,
        subfolder="transformer",
        revision=args.revision,
        sample_size=args.resolution // 8,
        interpolation_scale=args.resolution / 512,
        use_additional_conditions=args.use_additional_conditions,
    )

    # Convert PixArt transformer to SV-DRR transformer
    if (
        transformer.config.caption_channels is not None
        or transformer.config.in_channels != 8
    ):
        transformer_new = SvdrrTransformer2DModel.from_config(
            transformer.config,
        ...
        new_state_dict = {
            ...
            if k in transformer_new.state_dict()
            and v.shape == transformer_new.state_dict()[k].shape
        }
        transformer_new.load_state_dict(new_state_dict, strict=False)
        with torch.no_grad():
            torch.nn.init.zeros_(transformer_new.pos_embed.proj.weight)
            transformer_new.pos_embed.proj.weight[:, :4, :, :].copy_(
                transformer.pos_embed.proj.weight
            )
            transformer_new.pos_embed.proj.bias.copy_(transformer.pos_embed.proj.bias)
        transformer = transformer_new
        del transformer_new
    args.use_additional_conditions = (transformer.use_additional_conditions,)
    ...
    cc_projection = CCProjection(
        in_channel=image_encoder.config.projection_dim + 4,  # 768 + 4
        out_channel=transformer.config.cross_attention_dim,
    )
    torch.nn.init.eye_(
        list(cc_projection.parameters())[0][
            : image_encoder.config.projection_dim, : image_encoder.config.projection_dim
        ]  # 768, 768
    )
    ...
    if args.use_ema:
        ema_transformer = EMAModel(
            ...
            model_cls=SvdrrTransformer2DModel,
    ...
    if accelerator.unwrap_model(cc_projection).dtype != torch.float32:
        raise ValueError(
            f"Transformer loaded as datatype {accelerator.unwrap_model(cc_projection).dtype}. {low_precision_error_string}"
        )
    ...
    optimizer = optimizer_class(
        [
            ...
            {"params": cc_projection.parameters(), "lr": 10.0 * args.learning_rate},
        ],
    ...
    def print_model_info(model):
        ...
        accelerator.print("model name: ", type(model).__name__)
        # print("model: ", model)
        accelerator.print(
        ...
        accelerator.print(
            "total parameters(M): ", sum(p.numel() for p in model.parameters()) / 1e6
        )
        accelerator.print(
            "model size(MB): ",
    ...
    # LidcIdri Hard data
    train_dataset = LidcidriHardFBData(
        root_dir=args.train_data_dir,
        image_transforms=image_transforms,
        validation=False,
        CT_thickness=args.ct_thickness,  # "thick" or "thin" or "all"
        orientation=args.xray_orientation,  # "PA" or "AP" or "all"
    )
    validation_dataset = LidcidriHardFBData(
        root_dir=args.train_data_dir,
        image_transforms=image_transforms,
        validation=True,
        CT_thickness=args.ct_thickness,
        orientation=args.xray_orientation,
    )
    ...
    #         model=transformer, mode="reduce-overhead", fullgraph=True
    #     )
    ...
    if accelerator.is_main_process:
        ...
        if args.report_to == "wandb" and args.resume_wandb_run_id is not None:
            init_kwargs = {
                "wandb": {
                    "resume": "must",
                    "id": args.resume_wandb_run_id,
                    "config": tracker_config,  # do this because passing config to accelerator.init_trackers not always work
                }
            }
        else:
            init_kwargs = {
    ...
    logger.info(f" use_additional_conditions = {args.use_additional_conditions}")
    if torch.cuda.is_available():
        logger.info(
            f" torch.num_devices = {torch.cuda.device_count()}, {torch.cuda.get_device_name()}"
        )
    global_step = 0
    first_epoch = 0
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            inner_steps = global_step % num_update_steps_per_epoch
            train_dataloader.set_epoch(first_epoch)

        # if lr is changed, set it here (only for constant_with_warmup now )
        if args.learning_rate != optimizer.param_groups[0]["initial_lr"]:
            accelerator.print(
                f"Setting learning rate from {optimizer.param_groups[0]['lr']} to {args.learning_rate}"
            )

            optimizer.param_groups[0]["lr"] = optimizer.param_groups[0][
                "initial_lr"
            ] = args.learning_rate
            optimizer.param_groups[1]["lr"] = optimizer.param_groups[1][
                "initial_lr"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        loss_epoch = 0.0
        num_train_elems = 0
        if args.resume_from_checkpoint and epoch == first_epoch and inner_steps > 0:
            # Skip the first batches of the first epoch if we're resuming from a checkpoint
            active_dataloader = accelerator.skip_first_batches(
                train_dataloader, inner_steps
            )
        else:
            active_dataloader = train_dataloader
        for step, batch in enumerate(active_dataloader):
            # To debug dataloader, print pid and target_view
            if global_step == initial_global_step and args.debug:
                logger.info(
                    f"Data in the first step: {global_step}\n pid: {batch['pid']}\n target_view: {batch['target_view']}",
                    main_process_only=False,
                )

            with accelerator.accumulate(transformer, cc_projection):
                ...
                img_latents = (
                    vae.encode(input_image).latent_dist.mode().detach()
                )  # .sample()
                ...
                if (
                    ...
                    pose_prompt_embeds = _encode_pose(
                        pose, gt_latents.device, gt_latents.dtype, False
                    )  # [B, 1, 4]
                    ...
                    prompt_embeds = torch.cat(
                        [img_prompt_embeds, pose_prompt_embeds], dim=-1
                    )
                    prompt_embeds = cc_projection(prompt_embeds)
                    ...
                    prompt_embeds = _encode_image_with_pose(
                        image_encoder,
                        cc_projection,
                        input_image,
                        pose,
                ...
                # Not passing this to train_diffusion.training_losses_diffusers() because it can't handle img_latents
                # latent_model_input = torch.cat(
                #     [noisy_latents, img_latents], dim=1
                # )  # # [B,4+4,32,32]

                # Predict the noise residual
                # model_pred = transformer(
                #     hidden_states=latent_model_input,  # [B,8,32,32]
                #     encoder_hidden_states=prompt_embeds,  # [B, 1, 768]
                ...
                loss_term = train_diffusion.training_losses_diffusers(
                    ...
                    model_kwargs=dict(
                        ...
                        added_cond_kwargs={
                            ...
                            "aspect_ratio": 1,  # None,
                        },  # added_cond_kwargs only works when image size = 1024(vae latent=128) due to transformer implementation
                        latents_concat=img_latents,
                    ),
                    # img_cond=img_latents,
                )
                ...
                # split learned noise and sigma
                # noise_pred, sigma_pred = model_pred.chunk(2, dim=1)

                # frozen_out = torch.cat([noise_pred.detach(), sigma_pred], dim=1)
                # loss_vb = _vb_terms_bpd(
                #     frozen_out,
                #     x_start=gt_latents,
                #     x_t=noisy_latents,
                #     t=timesteps,
                ...
                if accelerator.sync_gradients:
                    params_to_clip = itertools.chain(
                        transformer.parameters(), cc_projection.parameters()
                    )
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    ...
                    if (
                        args.checkpoints_total_limit is not None
