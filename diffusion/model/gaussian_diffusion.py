    ...
    def __init__(
        ...
        *,
        ...
        if False:
            ...
            alpha_cuspord_rmove1 = np.concatenate(
                [np.ones([1]), alpha_cumprod_snr_shift[:999]]
            )
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.return_startx and self.model_mean_type == ModelMeanType.EPSILON:
                ...
                return (
                    output,
                    self._predict_xstart_from_eps(x_t=x_t, t=t, eps=output),
                    x_t,
                )
            ...
            if self.snr:
                if self.model_mean_type == ModelMeanType.START_X:
                    ...
                    pred_noise = output
                    pred_startx = self._predict_xstart_from_eps(
                        x_t=x_t, t=t, eps=output
                    )
            ...
            if model_kwargs.get("mask_ratio", False) and model_kwargs["mask_ratio"] > 0:
                ...
                loss = F.avg_pool2d(
                    loss.mean(dim=1), model.model.module.patch_size
                ).flatten(1)
                mask = model_output["mask"]
                unmask = 1 - mask
                terms["mse"] = (
                    mean_flat(loss * unmask) * unmask.shape[1] / unmask.sum(1)
                )
                if model_kwargs["mask_loss_coef"] > 0:
                    terms["mae"] = (
                        model_kwargs["mask_loss_coef"]
                        * mean_flat(loss * mask)
                        * mask.shape[1]
                        / mask.sum(1)
                    )
