...
# Copy from PixArtTransformer2DModel
class SvdrrTransformer2DModel(ModelMixin, ConfigMixin):
    r"""
    ...
    def __init__(
        ...
        if self.config.caption_channels is not None:
            ...
            )  # caption_channels=4096, hidden_size=1152
    ...
    def forward(
        ...
        Args:
            ...
            latents_concat: (N, C=4, H, W) tensor of encoded latent representations of condition images
        ...
        if latents_concat is not None:
            # concat img_latents to hidden_states along channel dimension
            assert (
                hidden_states.shape == latents_concat.shape
            ), "latents of x_0 image and condition image must have the same shape"
            hidden_states = torch.cat(
                [hidden_states, latents_concat], dim=1
            )  # # [B,4+4,S,S] S=32 for 256x256

        # ensure input tensor has the correct number of channels
        assert (
            hidden_states.shape[1] == self.config.in_channels
        ), f"Input tensor must have {self.config.in_channels} channels, but has {hidden_states.shape[1]}."
